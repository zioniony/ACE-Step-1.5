...
比如在 **Remix 模式**中，你用参考音频来约束作曲、和弦、结构，直接让 DiT 去生成。这时，**你取代了 LM 的工作**——你自己成为了规划者。
...
| | `audio_codes` | Remix模式下输入模型的语义 codes（高级用法：复用 codes 生成变体、将歌曲转 codes 拓展衍生、拼接组合像 DJ 一样混搭） |
...
源音频（`src_audio`）用于 **Remix（Cover）任务**，进行**旋律结构控制**。它的原理是将你输入的源音频量化成语义结构化的信息。
...
你可以用 Remix 去 **重新混搭一首歌**，且支持更改 Caption 和 Lyrics：
...
ACE-Step 1.5 的 Gradio 界面分为三个主要标签页：
...
**顶部：模式选择**
- Turbo 模型可用模式：`Simple`、`Custom`、`Remix`、`Repaint`
- Base 模型额外可用：`Extract`、`Lego`、`Complete`
- `Load File` 按钮：可加载保存的 JSON 格式生成参数配置

**Simple Mode（简单模式）**
- 一行简单的文本输入框，输入歌曲描述即可快速生成
- 支持选择人声语言和乐器模式
- `Random` 按钮：生成随机描述作为起点
- `Create Sample` 按钮：一键生成完整的歌曲样本（包含自动生成的 caption、lyrics 和 metas）

**Custom Mode（自定义模式）**
- **参考音频**：用于控制生成音乐的声学特征（音色、混音风格等）
- **Caption（描述）**：对音乐整体要素的描述（风格、乐器、情绪、氛围等）
- **Lyrics（歌词）**：时序要素的描述（歌词内容、结构标记、演唱方式等）
- **Format Caption** 按钮：使用 LM 自动扩展和优化你的描述
- **Format Lyrics** 按钮：使用 LM 自动格式化和优化歌词
- **Instrumental** 复选框：纯乐器模式
- **Sample** 按钮：生成随机描述和歌词作为模板

**LM Codes Hints（LM 代码提示）**
- 可上传参考音频或直接输入语义 codes
- `Convert to Codes` 按钮：将源音频转换为语义 codes
- `Transcribe` 按钮：转录音频为文本描述

**Retake & Edit（重生成与编辑）**
- **Retake（重生成）**：在保持相同参数的情况下生成变体
  - `variance`：控制变化程度（0=基线，0.05–0.15=微妙变化，0.5+=强烈变化）
  - `seed`：固定随机种子（留空=随机）
- **Edit（编辑）**：基于源音频进行 morphing 编辑
  - `Copy current → source`：将当前生成结果复制为源
  - `source caption`：描述原始音频的特征
  - `source lyrics`：原始音频的歌词（顶部 lyrics 为目标）
  - `n_min/n_max/n_avg`：控制编辑的精细程度

**可选参数（Optional Parameters）**
- **BPM**：速度（自动/手动控制）
- **Key Scale**：调性（自动/手动控制）
- **Time Signature**：拍号（自动/手动控制）
- **Vocal Language**：人声语言（自动/手动控制）
- **Duration**：目标时长（自动/手动控制，显示最大限制）
- **Batch Size**：批量生成数量（1–8，显示最大限制）
- `Reset All Auto` 按钮：重置所有自动控制

**生成行（Generate Row）**
- **Think**：启用 LM 的 Chain-of-Thought 推理（需已初始化 LM）
- **Auto Score**：自动为生成结果打分
- **Generate**：开始生成
- **AutoGen**：自动生成下一批次（后台持续生成）
- **Auto LRC**：自动生成歌词时间轴（LRC 文件）

**Remix Strength（Remix 强度）**
- `codes_strength`：控制源音频/codes 的影响强度（0.0–1.0），仅在 Remix 模式下可见

**Repainting Controls（重绘控制）**
- `repainting_start/end`：操作的时间区间（秒）
- `Repaint Mode`：保守/平衡/激进模式
- `Repaint Strength`：重绘强度（0=保守，1=激进）
...
- GPU 信息与等级显示
- DiT 模型选择
- 设备选择（auto/cuda/mps/xpu/cpu）
- VAE 模型选择
- LM 模型选择
- LM 后端选择（vllm/pt/mlx）
- 初始化开关：Flash Attention、Offload to CPU、Offload DiT to CPU、Compile Model、Quantization
- `Initialize Service` 按钮

**LoRA Controls（LoRA 控制）**
- 加载 LoRA 适配器
- LoRA 权重控制

**DiT Controls（DiT 控制）**
- 推理步数（inference_steps）
- CFG 强度（guidance_scale）
- Shift 参数
- 推理方法（ode/sde）

**LM Controls（LM 控制）**
- 采样温度（lm_temperature）
- LM CFG 强度（lm_cfg_scale）
- Top-K/Top-P 采样
- 负面提示（lm_negative_prompt）
...
- 训练参数设置（epoch、batch size、学习率等）
- 开始训练
- 训练进度显示
- 导出训练好的 LoRA 模型

**LoKR Training（LoKR 训练）**
- 低秩 Kronecker 积训练选项
...
适合快速起步和探索。只需输入一句简单的歌曲描述，系统会自动为你生成完整的 caption、lyrics 和音乐元数据。

**使用流程：**
1. 在输入框中输入歌曲描述（如："一首悲伤的钢琴民谣"）
2. 选择人声语言或勾选 Instrumental（纯乐器）
3. 点击 `Create Sample` 按钮，系统会自动生成完整的参数并进行生成

**特点：**
- 零门槛：无需了解复杂参数
- 快速：一句话即可开始创作
- 智能：LM 自动扩展你的描述为完整的创作方案

### Custom（自定义模式）

适合有明确创作意图的用户，可以精细控制每一个参数。

**核心输入：**
- **Caption**：描述音乐的整体特征（风格、情绪、乐器、音色等）
- **Lyrics**：控制音乐的时间演进（歌词、结构标记、能量变化等）
- **Reference Audio**：参考音频，控制声学特征

**辅助功能：**
- `Format Caption`：让 LM 帮你扩展和优化描述
- `Format Lyrics`：让 LM 帮你格式化和优化歌词
- `Sample`：生成随机示例作为参考

### Remix（风格迁移 / Cover）

> **注意**：UI 中的 "Remix" 模式对应底层任务类型 `cover`。之前版本中称为 "Cover" 模式，现已更名为 "Remix"。

基于源音频的结构进行风格迁移，保持旋律和和弦进行，改变风格和细节。

**使用流程：**
1. 上传源音频（`src_audio`）
2. 点击 `Analyze` 按钮分析音频
3. 修改 Caption 和 Lyrics 来重新诠释原曲
4. 通过 `codes_strength` 控制源音频结构的遵循程度

**高级技巧：**
- 改变风格：从 pop 改为 rock 或 jazz
- 改变歌词：用新的歌词重新填词
- 改变情绪：通过 Caption 调整整体氛围

### Repaint（局部重绘）

基于源音频的上下文进行补全或修改，支持局部修改、续写、改歌词等操作。

**使用流程：**
1. 上传源音频（`src_audio`）
2. 设置重绘区间（`repainting_start/end`）
3. 选择重绘模式（保守/平衡/激进）
4. 修改 Caption 和 Lyrics 来控制重绘内容

**应用场景：**
- **局部修改**：修改指定区间的歌词或结构
- **续写**：基于上下文续写开头或结尾
- **改歌词**：保持旋律和配器，只改变歌词内容
- **无限时长生成**：通过多次 Repaint 操作实现无限时长生成

### Lego（轨道添加）

> **仅 Base 模型可用**

基于已有轨道智能添加新轨道，新轨道会与原有轨道在节奏、和声上协调配合。

**使用流程：**
1. 上传源音频（如人声轨）
2. 设置操作区间
3. 通过 Caption 描述要添加的乐器类型
4. 生成新的伴奏轨道
...
给单轨添加完整的混合伴奏，生成的伴奏会与人声在风格、节奏、和声上匹配。

**使用流程：**
1. 上传单轨音频（如清唱人声）
2. 通过 Caption 描述期望的风格和配器
3. 生成完整的混合伴奏
...
Retake 允许你在保持相同参数的情况下生成变体，通过引入受控的噪声变化来探索创作空间。

**参数说明：**

| 参数 | 范围 | 说明 |
|------|------|------|
| `variance` | 0.0–1.0 | 控制变化程度，0=基线（几乎不变），0.05–0.15=微妙变化，0.5+=强烈变化 |
| `seed` | 任意整数 | 固定随机种子，留空则随机 |

**使用场景：**
- 对当前生成结果基本满意，但想尝试一些微小变化
- 在保持相同结构的前提下探索不同的细节表现
- 快速生成多个相似但不同的版本进行比较

**⚠️ Think 模式警告：**
当启用 Think 模式时，Retake 会同时引入 LM 的漂移和噪声漂移。如果想要干净地重生成 Think 模式的结果：
1. 打开结果的「Score & LRC & LM Codes」面板
2. 复制其 **LM Codes** 到「LM Codes Hints」字段
3. 取消勾选 Think
4. 再调整 variance 进行重生成

### Edit（编辑）

Edit 是一种基于 morphing 的高级编辑功能，允许你描述"源状态"和"目标状态"，让模型在两者之间进行平滑过渡。

**参数说明：**

| 参数 | 范围 | 说明 |
|------|------|------|
| `source caption` | 文本 | 描述原始音频的特征 |
| `source lyrics` | 文本 | 原始音频的歌词 |
| `n_min` | 0.0–1.0 | 最小编辑强度 |
| `n_max` | 0.0–1.0 | 最大编辑强度 |
| `n_avg` | 1–8 | 平均编辑步长 |

**使用流程：**
1. 先生成一个基础版本
2. 点击 `Copy current → source` 将当前结果复制为源
3. 修改顶部的 Caption 和 Lyrics 为目标状态
4. 调整 n_min/n_max/n_avg 控制编辑精细程度
5. 点击 Generate 进行编辑生成

**应用场景：**
- 渐进式风格变化：从柔和过渡到强烈
- 歌词替换：保持旋律但更换歌词内容
- 配器调整：在保持结构的前提下增减乐器层次
...
启用 LM 的 Chain-of-Thought 推理，让 LM 自动完成以下工作：

- 推理音乐元数据（BPM、调性、时长等）
- 优化和扩展你的 caption
- 生成语义 codes（包含作曲旋律、配器编排、少许音色信息）
...
**何时关闭：**
- 你已经有明确的规划目标
- 你使用 Remix/Repaint 模式（此时你自己成为规划者）
- 你追求最快的生成速度

### Auto Score（自动打分）

启用后，系统会自动为每个生成结果计算质量分数，包括：

- **DiT Lyrics Alignment Score**：歌词与音频的对齐程度
- 其他质量评估指标

**使用建议：**
- 批量生成时启用，帮助快速筛选高质量结果
- 配合 AutoGen 使用，持续生成并自动打分

### AutoGen（自动生成）

启用后，系统会在后台持续生成新的批次，使用相同的参数但随机的 seed。

**工作机制：**
- 当你查看当前批次结果时，下一批次已经在后台生成
- 每个新批次使用新的随机 seed，确保结果多样性
- 支持批量探索随机空间，提高发现优秀结果的概率

**使用建议：**
- 设置较大的 batch_size（如 4 或 8）
- 配合 Auto Score 使用，自动筛选高分结果
- 开启后可以"边听边生成"，提高创作效率

### Auto LRC（自动歌词时间轴）

启用后，系统会自动为生成的歌曲生成 LRC 格式的歌词时间轴。
...
每个生成结果都有独立的音频播放器，支持播放、暂停、下载等操作。

### 快捷操作按钮

| 按钮 | 功能 |
|------|------|
| `Send to Remix` | 将此结果作为源音频发送到 Remix 模式 |
| `Send to Repaint` | 将此结果作为源音频发送到 Repaint 模式 |
| `Save` | 保存此音频文件 |

### Score & LRC & LM Codes 面板

展开后显示详细信息：

**LM Codes**
- 显示 LM 生成的语义 codes
- 可复制用于后续生成（如在 Retake 中保持相同结构）
- `Convert to Codes` 按钮：将音频转换为 codes
...
- 显示歌词时间轴
- 支持手动编辑
- `Generate LRC` 按钮：生成歌词时间轴
- `Save LRC` 按钮：导出 LRC 文件

### 批次管理

支持多批次生成和切换：
- `Prev Batch` / `Next Batch`：切换批次
- `Batch Indicator`：显示当前批次/总批次
- `Next Batch Status`：显示后台生成状态
- `Restore Params`：一键恢复当前批次的生成参数

### 批量结果

- `Generated Audio Batch`：显示当前批次所有文件
- `Generation Info`：显示生成参数详情（模型、步数、CFG 等）
